...
df %>% 
  select(hour, p_value, eta2, highest_level, lowest_level) %>% 
  mutate(
    across(c(eta2, p_value), \(x) signif(x, 2)),
    across(ends_with("level"), \(x) wday(as.numeric(x), label = TRUE))) %>% 
  write_csv("supp/Table_S1.csv")
...
df_wday <- btc_index_deviations %>%
  filter(
    ...
    lubridate::year(time) >= first_year, 
    lubridate::year(time) <= last_year,
    lubridate::hour(time) == 13
    ) %>%
  mutate(wday = lubridate::wday(time, label = TRUE))

stats_wday_13 <- summarize_factor(df_wday, wday)
```
...
btc_index_deviations %>% filter(
  ...
  simple_stats(wday = wday(time, label = TRUE)) %>% 
  ggplot() +
  geom_hline(yintercept = 0, color = "red") +
...
btc_index_deviations %>% filter(
  ...
  simple_stats(wday = wday(time, label = TRUE), year = year(time)) %>% 
  ggplot() +
  geom_hline(yintercept = 0, color = "red") +
...
To test stability, I recomputed the hour/weekday/month summaries over 2014–2023, 2019–2023, and 2022–2023. Each window repeats the same methodology. The results shown in the table below emphasize the earlier conclusions. Hour and weekday remain negligible in every window. The day-of-month effect that is weakly present over 2014–2023 does not replicate in 2022–2023 (large *p*, $\eta^{2}$ ≈ 0), indicating that any apparent month-end dip is regime-dependent rather than a durable pattern.
...
bind_rows(
  ...
  df_wday %>% mutate(factor = "Day of week (00:00 UTC)"),
  df_dom %>% mutate(factor = "Day of month (00:00 UTC)")) %>% 
  select(-data) %>% 
